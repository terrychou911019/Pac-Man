...
extern ALLEGRO_SAMPLE* PACMAN_EATFRUIT;
extern ALLEGRO_SAMPLE* GAMEOVER;
extern ALLEGRO_SAMPLE* YOU_WIN;
extern ALLEGRO_SAMPLE* PLAYING;
extern ALLEGRO_SAMPLE* PLAYINGB;
extern ALLEGRO_SAMPLE* PLAYINGC;
	...
	if (game_over) {
		...
		*/
		al_draw_scaled_bitmap(pman->die_sprite,
			al_get_timer_count(pman->death_anim_counter) / 10 * 16, 0,
			16, 16,
...
void pacman_eatItem(Pacman* pacman, const char Item) {
	...
	case 'P':
		stop_bgm(PACMAN_MOVESOUND_ID);
		PACMAN_MOVESOUND_ID = play_audio(PACMAN_EATFRUIT, effect_volume);
		break;
	default:
		break;
	}
